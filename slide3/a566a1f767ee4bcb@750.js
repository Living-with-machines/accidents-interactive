...
function _dataFrame(d3,Plot,place,accumulativeByDate,width,htl)
{
...
function _place(Inputs,m,htl,d3,Event)
{
  ...
  return Object.assign(htl.html`${embedding}`, { value: "all" })
}
...
async function* _counter(reloadComment,setHTML,delaySeconds,delayFrames,Promises,location)
{
  let frame = 0,
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("dataFrame")).define("dataFrame", ["d3","Plot","place","accumulativeByDate","width","htl"], _dataFrame);
  main.variable(observer("viewof place")).define("viewof place", ["Inputs","m","htl","d3","Event"], _place);
  ...
  main.variable(observer("counter")).define("counter", ["reloadComment","setHTML","delaySeconds","delayFrames","Promises","location"], _counter);
